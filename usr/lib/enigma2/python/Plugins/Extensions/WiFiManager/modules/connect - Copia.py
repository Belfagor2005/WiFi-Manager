...
class WiFiConnectZ(Screen):
    ...
    def handle_connect_after_password(self, answer, callback):
        ...
        if answer:
            self.execute_connection_with_callback(callback)
        else:
            if callback:
                callback()
    ...
    def show_message(self, message, callback=None):
        """Show message with proper callback handling - FIXED"""
    ...
    def show_network_options(self):
        """ChoiceBox with all possible options - STAY IN OPTIONS"""
        ...
        if current_index is not None and current_index < len(self.networks):
            ...
            def choice_callback(choice):
                ...
                if action == "back":
                    ...
                    print("[DEBUG] Connect selected - will stay in options")

                    def after_connect():
                        # Refresh the interface status
                        self.refresh_after_connection()
                        # Show options again with updated status
                        reactor.callLater(2, self.show_network_options)

                    self.execute_connection_with_callback(after_connect)

                elif action == "disconnect":
                    print("[DEBUG] Disconnect selected - will stay in options")
                    ...
                    self.open_password_dialog_with_callback(lambda: self.show_network_options())

                elif action == "edit_config":
                    print("[DEBUG] Edit config selected")
                    self.open_configuration_with_callback(lambda: self.show_network_options())

                elif action == "forget":
                    print("[DEBUG] Forget selected")
                    self.forget_network_with_callback(lambda: self.show_network_options())

                elif action == "info":
                    print("[DEBUG] Info selected")
                    self.show_connection_details_with_callback(lambda: self.show_network_options())

            # Apri il ChoiceBox con il callback
