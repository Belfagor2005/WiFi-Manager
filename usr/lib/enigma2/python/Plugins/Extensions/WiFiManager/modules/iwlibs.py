...
from .flags import (
    ...
    SIOCGIWMODE, SIOCSIWFREQ, SIOCGIWFREQ, SIOCSIWENCODE, SIOCGIWENCODE,
...
class Wireless(object):
    ...
    def setAPaddr(self, addr):
        ...
        if (addr == "AUTO" or addr == "ANY"):
            mac_addr = b"\xFF" * ETH_ALEN
        elif addr == "OFF":
            mac_addr = b'\x00' * ETH_ALEN
        else:
            if ":" not in addr:
                # not a hardware address
                raise IOError(errno.ENOSYS, strerror(errno.ENOSYS))
    ...
    def getEncryption(self):
        ...
        iwpoint = Iwpoint(b'\x00' * IW_ENCODING_TOKEN_MAX)
        status, result = self.iwstruct.iw_get_ext(self.ifname,
                                                  SIOCGIWENCODE,
                                                  data=iwpoint.packed_data)
        iwpoint.update(result)

        if iwpoint.flags & IW_ENCODE_NOKEY > 0:
            return '**' * iwpoint.length
        elif iwpoint.flags & IW_ENCODE_OPEN > 0:
            return 'open'
        elif iwpoint.flags & IW_ENCODE_RESTRICTED > 0:
            return 'restricted'
        elif iwpoint.flags & IW_ENCODE_DISABLED > 0:
            return 'off'
    ...
    def setEncryption(self, mode):
        ...
        if mode == 'OPEN' or mode == IW_ENCODE_OPEN:
            numeric_mode |= IW_ENCODE_OPEN
        elif mode == 'RESTRICTED' or mode == IW_ENCODE_RESTRICTED:
            numeric_mode |= IW_ENCODE_RESTRICTED
        elif mode == 'OFF' or mode == IW_ENCODE_DISABLED:
            numeric_mode |= IW_ENCODE_DISABLED

        iwpoint = Iwpoint(b'\x00' * IW_ENCODING_TOKEN_MAX, numeric_mode)
    ...
    def setKey(self, key, index=1):
        ...
        """
        if index not in range(1, IW_ENCODE_INDEX):
            raise IndexError
    ...
    def setFrequency(self, freq):
        ...
        if freq == "auto":
            ...
            if e > 8:
                ...
                m = int(freq_num)
                e = 0
            iwreq = iwstruct.pack(format, m, e, 0, IW_FREQ_FIXED)
    ...
    def getMode(self):
        ...
        """
        return modes[self.wireless_info.getMode()]
...
class WirelessConfig(object):
    ...
    def getKey(self, key=0):
        ...
        # use an IW_ENCODING_TOKEN_MAX-cell array of NULLs
        #   as space for ioctl to write encryption info
        iwpoint = Iwpoint(b'\x00' * IW_ENCODING_TOKEN_MAX, key)
        status, result = self.iwstruct.iw_get_ext(
            self.ifname,
            SIOCGIWENCODE,
            data=iwpoint.packed_data
    ...
    def getEssid(self):
        ...
        iwpoint = Iwpoint(b'\x00' * IW_ESSID_MAX_SIZE)
        status, result = self.iwstruct.iw_get_ext(
            self.ifname,
            SIOCGIWESSID,
            data=iwpoint.packed_data
...
class WirelessInfo(WirelessConfig):
    ...
    def getSensitivity(self):
        ...
        """
        return Iwparam(self.ifname, SIOCGIWSENS)
    ...
    def getBitrate(self):
        ...
        """
        return Iwparam(self.ifname, SIOCGIWRATE)
    ...
    def getRTS(self):
        ...
        """
        return Iwparam(self.ifname, SIOCGIWRTS)
    ...
    def getFragmentation(self):
        ...
        """
        return Iwparam(self.ifname, SIOCGIWFRAG)
    ...
    def getPower(self):
        ...
        """
        return Iwparam(self.ifname, SIOCGIWPOWER)
    ...
    def getTXPower(self):
        ...
        """
        return Iwparam(self.ifname, SIOCGIWTXPOW)
    ...
    def getRetry(self):
        ...
        """
        return Iwparam(self.ifname, SIOCGIWRETRY)
...
class Iwstruct(object):
    ...
    def iw_get_ext(self, ifname, request, data=None):
        ...
        try:
            ...
            return (result, ifreq[IFNAMSIZE:])
        except Exception as e:
            print(f"[DEBUG] iw_get_ext ERROR: {e}")  # DEBUG
            raise
...
class Iwrange(object):
    ...
    def __init__(self, ifname):
        self.fmt = (
            ...
            + IW_MAX_BITRATES * "i"
            + "2i2i2i2i3H"
            + IW_MAX_ENCODING_SIZES * "H"
            + "2BBHB"
            + IW_MAX_TXPOWER * "i"
            + "2B3H2i2iHB"
            + IW_MAX_FREQUENCIES * "ihBB"
            + "IiiHiI"
        )
        ...
        while (len(data) >= IW_EV_LCP_PK_LEN):
            ...
            if cmd == SIOCGIWAP:
                ...
                if ctypes.sizeof(ctypes.c_voidp) == 4:
                    ...
                    scanresult_len = IW_EV_LCP_PK_LEN
                else:
        ...
        if ((cmd in range(SIOCIWFIRST,
            ...
            if cmd == SIOCGIWNWID:
                ...
                if self.encode.caddr_t is None:
                    self.encode.flags |= IW_ENCODE_NOKEY
                ...
                raise ValueError(
                    "Unknown IW event command received. "
                    "This command cannot be used to add information "
